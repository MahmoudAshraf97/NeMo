...
class AudioNoiseBatch:
    ...
    def pin_memory(self):
        # Called by the DataLoader pin-memory thread (torch checks for this method on
        # custom batch types); without it `pin_memory=True` is silently a no-op and
        # every host-to-device copy of the large audio tensors is a slow pageable copy.
